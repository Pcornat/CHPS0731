...
Phong::computeColour(const Intersection &I, const glm::highp_dvec3 &point, const Scene &s, const Rayon &rayon,
    ...
    glm::highp_dvec3 amb(0, 0, 0), diff(0, 0, 0), spec(0, 0, 0), R, L, Li, Ld, Ltmp, refl(0, 0, 0), min(0, 0,
                                                                                                        0), max(
            1, 1, 1);
    ...
    for (auto light : s.Lights) {
        ...
        if (light->getType() == 1) {
            ...
            if (rayShadow.shadowRay(s, glm::distance(point, Li), rec - 1)) {
                for (int i = 0; i < planLight->getWidth() / planLight->getSampleStep(); i++) {
                    Li.x += planLight->getSampleStep();
                    Li.y = Ltmp.y;
                    for (int j = 0; j < planLight->getHeight() / planLight->getSampleStep(); j++) {
                        Li.y += planLight->getSampleStep();
                        Ld = glm::normalize(point - Li);
