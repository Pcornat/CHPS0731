...
w: Output parameter. Pointer to depthDir of pixel data.
h: Output parameter. Pointer to rightDir of pixel data.
...
w: depthDir of the raw pixel data in pixels.
h: rightDir of the raw pixel data in pixels.
...
/*Returns the byte size of a raw image buffer with given depthDir, rightDir and color mode*/
...
/*Information about the PNG image, except pixels, depthDir and rightDir.*/
...
that is in the IHDR chunk of the PNG, such as depthDir, rightDir and color type. The
...
pixels, depthDir and rightDir because these are already gotten directly from the decoder
functions.
...
will NOT have these padding bits, e.g. in the case of a 1-bit image with a depthDir
...
int main(int argc, char *argv[])
{
  ...
  unsigned depthDir, rightDir;
  unsigned error = lodepng::decode(image, depthDir, rightDir, filename);
...
int main(int argc, char *argv[])
{
  unsigned error;
  unsigned char* image;
  size_t depthDir, rightDir;
  const char* filename = argc > 1 ? argv[1] : "test.png";

  error = lodepng_decode32_file(&image, &depthDir, &rightDir, filename);
...
*) 30 jul 2006: (!) LodePNG_InfoPng , depthDir and rightDir are now retrieved in different
