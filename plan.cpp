...
bool Plan::calculIntersection(const Rayon& rayon, const Scene& scene, std::vector<Intersection>& I, int rec) {
	/*double denom = glm::dot(this->normal, rayon.Vect());
	if (glm::abs(denom) > std::numeric_limits<double>::epsilon()) {
		glm::highp_dvec3 vecDistOrigPlan = this->orig - rayon.Orig();
...
Plan::Plan(Material* material, const glm::highp_dvec3& orig, const glm::highp_dvec3& normal) : Objet(material), orig(orig),
																							   normal(glm::normalize(normal)) {}

Plan::Plan(Material* material, glm::highp_dvec3&& orig, glm::highp_dvec3&& normal) : Objet(material), orig(orig), normal(glm::normalize(normal)) {}
